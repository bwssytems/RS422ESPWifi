...
static const char *TAG_RS422 = "RS422";

String _deviceName;
char _networkClient[40];
int _portNumber;
int _baudRate;
bool _portNumberChanged = true;
bool _connectHadError = false;
...
// Initialize the Connection for the RS422 interface - GPIO 21 and 22 for pico
#define EX_UART_NUM UART_NUM_1
#define EX_UART_TXD (GPIO_NUM_21)
#define EX_UART_RXD (GPIO_NUM_22)
...
void uart_event_task(void *pvParameters)
{
  ...
  int pos;
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_DEBUG, TAG_UART, "uart event task entry");
  for (;;)
  {
    ...
    if (xQueueReceive(uart0_queue, (void *)&event, (portTickType)portMAX_DELAY))
    {
      ...
      case UART_DATA:
        ...
        ESP_LOG_LEVEL_LOCAL(ESP_LOG_DEBUG, TAG_UART, "[DATA EVT]: %s", dtmp);
...
void setup()
{
...
#endif
  Serial.begin(115200);
  esp_log_level_set(TAG_RS422, ESP_LOG_VERBOSE);
  esp_log_level_set(TAG_UART, ESP_LOG_VERBOSE);
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Setup started.....");
  ...
  //  }
  _networkClient[0] = 0;
  preferences.begin("rs422espwifi", true);
  _baudRate = preferences.getInt("rs422.rate", 4800);
  _portNumber = preferences.getInt("rs422.port", 9999);
  netclen = preferences.getBytes("rs422.client", _networkClient, (size_t)40);
  preferences.end();
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_DEBUG, TAG_RS422, "Loaded Prefs are %d, %d, <%s>", _baudRate, _portNumber, _networkClient);

  if (strlen(_networkClient) > 0)
  {
    _connectClient = true;
  }
  else
  ...
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "%s RS422 to Wifi", _deviceName.c_str());

  _wifiManager = new WiFiManager();
  _wifiManager->setEspLogLevel(ESP_LOG_INFO);
  char port_string[10];
  ...
  _wifiManager->addParameter(new WiFiManagerParameter("network_server", "Network Server", _networkClient, 40, "type=\"tel\"", WFM_LABEL_BEFORE));
  _wifiManager->addParameter(new WiFiManagerParameter("network_port", "Network Port", port_string, 5, "type=\"tel\"", WFM_LABEL_BEFORE));
  _wifiManager->addParameter(new WiFiManagerParameter("baud_rate", "Baud Rate", rate_string, 7, "type=\"tel\"", WFM_LABEL_BEFORE));
  ...
  {
    ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Wifi Connected to router");
    _wifiConnected = true;
    _wifiManager->startConfigPortal(_deviceName.c_str(), "networkdev1.");
    ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "Wifi Manager Portal started.....");
    if (strlen(_networkClient) == 0)
    {
      _startListenServer();
    }
    ...
    {
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "Setup not starting listen server.");
    }
  }
  else
  {
  ...
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Setup and configure RS422 UART.");
...
void loop()
{
  ...
  if (_connectClient)
  {
    ...
    {
      delay(5000);
      _clientNetwork->close(true);
      _clientNetwork->free();
      _clientNetwork = NULL;
      delay(5000);
    }

    if (_connectNetworkClient(_networkClient, _portNumber))
    {
      _connectClient = false;
      _netClientConnected = false;
    }
    ...
    {
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Network server could not connect Ip (%s) Retrying in 10 secs...", _networkClient);
      _connectHadError = true;
    }
    yield();
  }
  delay(100);
...
void _saveConfigCallback()
{
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Should save config");
...
void _saveParamsCallback()
{
  ...
  setPrefs(getParameterValue("baud_rate"), getParameterValue("network_port"), getParameterValue("network_server"));
  if (old_portNum != _portNumber || old_client.compareTo(String(_networkClient)))
  {
...
void setPrefs(const char *baud_rate, const char *port, const char *server)
{
  ...
  strcpy(_networkClient, server);
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_DEBUG, TAG_RS422, "Set Prefs are %d, %d, <%s>", _baudRate, _portNumber, _networkClient);

  if (preferences.begin("rs422espwifi", false))
  {
    preferences.putInt("rs422.rate", _baudRate);
    preferences.putInt("rs422.port", _portNumber);
    preferences.putBytes("rs422.client", _networkClient, strlen(_networkClient) + 1);
  }
...
bool _startListenServer()
{
  ...
  _listenServer->begin();
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "Listen Server started.....");
  return true;
}
...
bool _connectNetworkClient(const char *client_address, int client_port)
{
  ...
  if(_clientNetwork == NULL)
  {
    if (networkServer.fromString(client_address))
    {
      ...
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onConnect");
      _clientNetwork->onConnect([](void *r, AsyncClient *c) { _clientConnectedCallback(); }, NULL);
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onError");
      _clientNetwork->onError([](void *r, AsyncClient *c, int8_t error) { _onError(error); }, NULL);
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onAck");
      _clientNetwork->onAck([](void *r, AsyncClient *c, size_t len, uint32_t time) { _onAck(len, time); }, NULL);
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onDisconnect");
      _clientNetwork->onDisconnect([](void *r, AsyncClient *c) { _onDisconnect(); delete c; }, NULL);
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onTimeout");
      _clientNetwork->onTimeout([](void *r, AsyncClient *c, uint32_t time) { _onTimeout(time); }, NULL);
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onData");
      _clientNetwork->onData([](void *r, AsyncClient *c, void *buf, size_t len) { _onData(buf, len); }, NULL);
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork set onPoll");
      _clientNetwork->onPoll([](void *r, AsyncClient *c) { _onPoll(); }, NULL);
    }
    else
    {
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_WARN, TAG_RS422, "Client to network not not valid client address (%s)", client_address);
      _connectClient = false;
      return false;
    }
  }
  ...
  if(_clientNetwork != NULL)
  {
    ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "clientNetwork do connect");
    if (_clientNetwork->connect(networkServer, client_port))
    {
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Client to network connect called, wait for connect...");
      yield();
      return true;
    }
    else
    {
      ESP_LOG_LEVEL_LOCAL(ESP_LOG_WARN, TAG_RS422, "Client to network not connected (%s)", client_address);
    }
  }
  return false;
}
...
void _clientConnectedListenCallback(void *args, AsyncClient *theClient)
{
  if (!_netClientConnected)
  {
    ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Client to network connected - Configured network connection (%s)", theClient->remoteIP().toString().c_str());
  ...
  {
    ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "Client from listen server already onnected - Closing connection (%s)", theClient->remoteIP().toString().c_str());
    theClient->close();
  }
}
...
void _clientConnectedCallback()
{
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_INFO, TAG_RS422, "Client to network connected - Configured network connection (%s)", _clientNetwork->remoteIP().toString().c_str());
  _netClientConnected = true;
  _connectClient = false;
  _connectHadError = false;
}
...
void _onError(int8_t error)
{
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_ERROR, TAG_RS422, "Client network (%s:%d) recvd error of %s", _networkClient, _portNumber, _clientNetwork->errorToString(error));
  if (_listenServer == NULL)
    _connectClient = true;
  else
    _connectClient = false;
...
void _onData(void *buf, size_t len)
{
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "client network recvd data for length of %d", len);
  digitalWrite(_xmitLedPin, HIGH);
...
void _onPoll()
{
  // ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "client network recvd poll");
  _onAck(0, 0);
}
...
void _onAck(size_t len, uint32_t time)
{
  ...
  // ESP_LOG_LEVEL_LOCAL(ESP_LOG_VERBOSE, TAG_RS422, "client network recvd ack of %d", len);
  _ackedLength += len;
...
void _onTimeout(uint32_t time)
{
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_ERROR, TAG_RS422, "Client network timeout, setup retry..");
  if (_listenServer == NULL)
    _connectClient = true;
  else
    _connectClient = false;
...
void _onDisconnect()
{
  ESP_LOG_LEVEL_LOCAL(ESP_LOG_WARN, TAG_RS422, "Client network recvd disconnect, setup retry..");
  if (_listenServer == NULL)
    _connectClient = true;
  else
    _connectClient = false;
